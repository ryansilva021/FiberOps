...
const CS = {
  card:    { background: 'rgba(13,21,40,0.9)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: 20 },
  label:   { display: 'block', fontSize: 11, color: '#94a3b8', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 5 },
  input:   { width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '8px 12px', color: '#e2e8f0', fontSize: 13, outline: 'none', boxSizing: 'border-box' },
  select:  { width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '8px 12px', color: '#e2e8f0', fontSize: 13, outline: 'none', boxSizing: 'border-box', cursor: 'pointer' },
  ...
  modeBtn: (ativa) => ({
    flex: '1 1 120px', padding: '10px 12px', borderRadius: 8, border: `1px solid ${ativa ? '#6366f144' : 'rgba(255,255,255,0.08)'}`,
    cursor: 'pointer', fontWeight: 600, fontSize: 13,
    background: ativa ? '#6366f122' : 'rgba(255,255,255,0.04)',
    color: ativa ? '#818cf8' : '#64748b', transition: 'all .15s',
  }),
}
...
function Campo({ label, value, onChange, min, max, step = 0.01, unit }) {
  return (
    <div>
      <label style={CS.label}>{label}{unit && <span style={{ color: '#475569', fontWeight: 400 }}> ({unit})</span>}</label>
    ...
    <div>
        ...
        {Object.entries(SPLITTER_DB).map(([s, db]) => {
          ...
          return (
            <button key={s} type="button" title={`${s} → ${db} dB`}
              ...
              style={{ padding: '4px 10px', borderRadius: 6, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600, background: ativo ? '#6366f122' : 'rgba(255,255,255,0.04)', color: ativo ? '#818cf8' : '#64748b', border: `1px solid ${ativo ? '#6366f144' : 'rgba(255,255,255,0.08)'}` }}>
              {s}
            </button>
          )
        })}
      ...
      {selected.length > 0 && (
        <div style={{ color: '#475569', fontSize: 10 }}>
          {selected.map(s => `${s}→${SPLITTER_DB[s]}dB`).join(' + ')} = <b style={{ color: '#64748b' }}>{totalDB.toFixed(1)} dB</b>
        </div>
      )}
    </div>
  )
...
function CartaoResultado({ r, titulo, tech }) {
  return (
    <div style={{ ...CS.card, borderColor: `${r.cor}55` }}>
      {titulo && <div style={{ color: '#94a3b8', fontSize: 12, fontWeight: 600, marginBottom: 10 }}>{titulo}</div>}
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16 }}>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 11, color: '#64748b', marginBottom: 2 }}>Potência recebida</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 8 }}>
        {[['Fibra', r.perdaFibra], ['Fusões', r.perdaFusoes], ['Conectores', r.perdaConectores], ['Splitter(s)', r.perdaSplitters], ['Total', r.perdaTotal]].map(([label, val]) => (
          <div key={label} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 6, padding: '6px 10px' }}>
            ...
            <div style={{ color: label === 'Total' ? '#e2e8f0' : '#94a3b8', fontWeight: label === 'Total' ? 700 : 500, fontSize: 13 }}>{val.toFixed(2)} dB</div>
          </div>
        ))}
      </div>
      {tech && (
        <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {[
            ['Orçamento óptico', `${tech.budget} dB`, '#64748b'],
            ['Perdas totais',    `${r.perdaTotal.toFixed(2)} dB`, '#94a3b8'],
            ['Margem disponível', `${(r.potFinal - tech.rxMin).toFixed(2)} dB`, r.potFinal >= tech.rxMin ? '#22c55e' : '#ef4444'],
          ].map(([l, v, c]) => (
            <div key={l} style={{ flex: '1 1 100px', background: 'rgba(255,255,255,0.03)', borderRadius: 6, padding: '6px 10px' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      ...
      {idx === 0 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
          ...
          <div>
            <div style={{ color: '#e2e8f0', fontWeight: 600, fontSize: 13 }}>{t.de}</div>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 40, flexShrink: 0 }}>
          <div style={{ width: 2, flex: 1, background: 'rgba(255,255,255,0.08)', minHeight: 8 }} />
          <div style={{ width: 8, height: 8, borderRadius: '50%', background: 'rgba(255,255,255,0.2)' }} />
          <div style={{ width: 2, flex: 1, background: 'rgba(255,255,255,0.08)', minHeight: 8 }} />
        </div>

        {/* Card do trecho */}
        <div style={{ flex: 1, background: 'rgba(255,255,255,0.03)', borderRadius: 10, padding: '10px 12px', border: '1px solid rgba(255,255,255,0.06)', marginBottom: 6 }}>
          ...
          {(portaLabel || fibraLabel) && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
              ...
              {t.conector && (
                <span style={{ padding: '2px 8px', borderRadius: 4, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: '#64748b', fontSize: 11 }}>
                  {t.conector}
                </span>
              )}
            </div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, fontSize: 11 }}>
            <span style={{ color: '#64748b' }}>
              fibra: <b style={{ color: '#94a3b8' }}>{t.perdaFibra?.toFixed(2) ?? '—'} dB</b>
            </span>
            ...
            {t.nFusoes > 0 && (
              <span style={{ color: '#64748b' }}>
                fusões: <b style={{ color: '#94a3b8' }}>{t.nFusoes} × {(t.perdaFusoes / t.nFusoes).toFixed(2)} = {t.perdaFusoes?.toFixed(2)} dB</b>
              </span>
            )}
            <span style={{ color: '#64748b' }}>
              conect.: <b style={{ color: '#94a3b8' }}>{t.nConectores} = {t.perdaConectores?.toFixed(2)} dB</b>
            </span>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 2 }}>
        ...
        <div>
          <div style={{ color: '#e2e8f0', fontWeight: 600, fontSize: 13 }}>{t.para}</div>
          {t.potFinal != null ? (
            ...
            <div style={{ fontSize: 11, color: '#475569' }}>aguardando cálculo</div>
          )}
        </div>
      </div>
    </div>
...
function ModoAutomatico({ topologia, projetoId, tech }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {totalCtos === 0 && (
        <div style={{ ...CS.card, color: '#94a3b8', fontSize: 13 }}>
      ...
      {/* Seleção */}
      <div style={CS.card}>
        <div style={{ color: '#e2e8f0', fontWeight: 700, marginBottom: 14 }}>Selecionar caminho</div>
      ...
      {/* Loading */}
      {carregando && (
        <div style={{ ...CS.card, color: '#64748b', fontSize: 13, textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
            {caminho.olt && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2, padding: '8px 12px', borderRadius: 8, background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.2)', minWidth: 110 }}>
                ...
                <span style={{ fontSize: 12, color: '#e2e8f0', fontWeight: 600 }}>{caminho.olt.id}</span>
                {caminho.olt.modelo && <span style={{ fontSize: 10, color: '#64748b' }}>{caminho.olt.modelo}</span>}
            ...
            {caminho.cdo && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2, padding: '8px 12px', borderRadius: 8, background: 'rgba(124,58,237,0.08)', border: '1px solid rgba(124,58,237,0.2)', minWidth: 110 }}>
                ...
                <span style={{ fontSize: 12, color: '#e2e8f0', fontWeight: 600 }}>{caminho.cdo.id}</span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2, padding: '8px 12px', borderRadius: 8, background: 'rgba(2,132,199,0.08)', border: '1px solid rgba(2,132,199,0.2)', minWidth: 110 }}>
              ...
              <span style={{ fontSize: 12, color: '#e2e8f0', fontWeight: 600 }}>{caminho.cto.id}</span>
              {caminho.cto.porta_cdo && <span style={{ fontSize: 10, color: '#38bdf8' }}>Porta {caminho.cto.porta_cdo}</span>}
              {caminho.cto.capacidade && <span style={{ fontSize: 10, color: '#64748b' }}>{caminho.cto.capacidade} portas</span>}
...
function ModoBalanceada({ tech }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={CS.card}>
        <div style={{ color: '#e2e8f0', fontWeight: 700, marginBottom: 14 }}>Parâmetros OLT / Fibra</div>
        <div style={CS.grid}>
      ...
      <div style={CS.card}>
        <div style={{ color: '#e2e8f0', fontWeight: 700, marginBottom: 14 }}>Trecho OLT → ONU</div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={CS.card}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
          <div style={{ color: '#e2e8f0', fontWeight: 700 }}>Caminhos ({caminhos.length})</div>
          <button onClick={() => { setCaminhos(cs => [...cs, { ...DEF_TRECHO, nome: `Caminho ${cs.length + 1}` }]); setResultado(null) }}
            style={{ padding: '6px 14px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(255,255,255,0.04)', color: '#94a3b8', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>
            + Adicionar
          </button>
        </div>
    ...
    <div style={{ width: '100%' }}>
      ...
      <div style={{ ...CS.card, padding: '10px 14px', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          <span style={{ color: '#64748b', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', whiteSpace: 'nowrap' }}>Tecnologia:</span>
          {Object.entries(PON_TECHS).map(([k, t]) => (
            <button key={k} onClick={() => setTechKey(k)}
              style={{ padding: '5px 12px', borderRadius: 6, border: `1px solid ${k === techKey ? t.cor + '66' : 'rgba(255,255,255,0.08)'}`, background: k === techKey ? t.cor + '18' : 'rgba(255,255,255,0.03)', color: k === techKey ? t.cor : '#64748b', cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>
              {t.nome}
            </button>
          ))}
          <span style={{ color: '#475569', fontSize: 11, marginLeft: 4 }}>
            Orcamento: <b style={{ color: '#94a3b8' }}>{tech.budget} dB</b> · OLT: <b style={{ color: '#94a3b8' }}>{tech.potOLT} dBm</b> · RX min: <b style={{ color: '#94a3b8' }}>{tech.rxMin} dBm</b>
          </span>
        </div>
      </div>
      ...
      <div style={{ ...CS.card, background: 'rgba(255,255,255,0.015)', padding: '14px 16px', marginTop: 16 }}>
        <div style={{ color: '#475569', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
          {Object.entries(SPLITTER_DB).map(([s, db]) => (
            <span key={s} style={{ color: '#475569', fontSize: 11 }}><b style={{ color: '#64748b' }}>{s}</b>: {db} dB</span>
          ))}
        </div>
      </div>
    </div>
